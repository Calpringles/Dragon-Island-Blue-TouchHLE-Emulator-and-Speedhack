...
// Minimal host Objective-C runtime bindings, used on macOS to refresh the
// current NSOpenGLContext's drawable before a buffer swap. See swap_window().
#[cfg(target_os = "macos")]
extern "C" {
    fn sel_registerName(name: *const std::os::raw::c_char) -> *const std::ffi::c_void;
    fn objc_msgSend(receiver: *mut std::ffi::c_void, sel: *const std::ffi::c_void);
}
...
fn size_for_orientation(
    ...
    // The window needs one pixel per *device* pixel, so a Retina guest needs
    // twice as many as it has points, on top of any scale hack.
    let scale_hack = scale_hack.get() * screen_scale;
    match orientation {
...
pub struct Window {
    ...
    fullscreen: bool,
    scale_hack: NonZeroU32,
    /// Device pixels per point, i.e. 2 for a Retina guest. Unlike `scale_hack`
    /// the guest knows about this and renders at the higher resolution itself.
    screen_scale: u32,
...
impl Window {
    ...
    pub fn new(
        ...
        let screen_scale: u32 = if options.retina { 2 } else { 1 };
        ...
        let mut window = if Self::rotatable_fullscreen() {
            ...
            let (width, height) =
                size_for_orientation(device_family, device_orientation, scale_hack, screen_scale);
            let window = video_ctx
                .window(title, width, height)
                .position_centered()
                .resizable()
...

        {
            ...
            log!(
                "RENDERDIAG window {}x{} pts, drawable {}x{} px (screen_scale {})",
                pw, ph, dw, dh, screen_scale
            );
        }
    ...
    pub fn poll_for_events(&mut self, options: &Options) {
        ...
        // harness: inject scripted taps (window pixel coords -> guest coords).
        // transform_input_coords is a nested fn below; items are in scope here.
        for tap in crate::harness::due_taps() {
            let coords = transform_input_coords(self, (tap.x, tap.y), false);
            let map = HashMap::from([(FingerId::Mouse, coords)]);
            self.event_queue.push_back(if tap.down {
        ...
        /// SDL reports mouse positions in window points. With a high-DPI
        /// drawable a point is more than one pixel, and the viewport that
        /// `transform_input_coords` works in is in pixels.
        fn mouse_point_to_pixel(window: &Window, (x, y): (f32, f32)) -> (f32, f32) {
            let (point_w, point_h) = window.window.size();
            let (pixel_w, pixel_h) = window.window.drawable_size();
            if point_w == 0 || point_h == 0 {
                return (x, y);
            }
            (
                x * (pixel_w as f32 / point_w as f32),
                y * (pixel_h as f32 / point_h as f32),
            )
        }
            ...
            self.event_queue.push_back(match event {
                ...
                E::MouseButtonDown {
                    ...
                    let coords = transform_input_coords(
                        self,
                        mouse_point_to_pixel(self, (x as f32, y as f32)),
                        false,
                    );
                ...
                E::KeyDown {
                    ...
                    let mut spd = crate::options::SPEED_MULTIPLIER.load(std::sync::atomic::Ordering::Relaxed);
                    if spd == 25 { spd = 50; }
                    else if spd == 50 { spd = 100; }
                    else if spd == 100 { spd = 150; }
                    else if spd == 150 { spd = 200; }
                    else if spd == 200 { spd = 300; }
                    else if spd == 300 { spd = 500; }
                    else if spd == 500 { spd = 1000; }
                    else if spd == 1000 { spd = 2000; }
                    crate::options::SPEED_MULTIPLIER.store(spd, std::sync::atomic::Ordering::Relaxed);
                    echo!("Speed Multiplier: {:.2}x", spd as f64 / 100.0);
                    continue;
                }
                E::KeyDown {
                    ...
                    let mut spd = crate::options::SPEED_MULTIPLIER.load(std::sync::atomic::Ordering::Relaxed);
                    if spd == 2000 { spd = 1000; }
                    else if spd == 1000 { spd = 500; }
                    else if spd == 500 { spd = 300; }
                    else if spd == 300 { spd = 200; }
                    else if spd == 200 { spd = 150; }
                    else if spd == 150 { spd = 100; }
                    else if spd == 100 { spd = 50; }
                    else if spd == 50 { spd = 25; }
                    crate::options::SPEED_MULTIPLIER.store(spd, std::sync::atomic::Ordering::Relaxed);
                    echo!("Speed Multiplier: {:.2}x", spd as f64 / 100.0);
                    continue;
                }
                E::KeyDown {
                    keycode: Some(sdl2::keyboard::Keycode::Backslash),
                    ..
                } => {
                    crate::options::SPEED_MULTIPLIER.store(100, std::sync::atomic::Ordering::Relaxed);
                    echo!("Speed Multiplier: 1.00x");
                    continue;
                }
                E::KeyDown {
    ...
    pub fn swap_window(&self) {
        // This performs a buffer swap via `[NSOpenGLContext flushBuffer]` on
        // macOS. Callers inside a coroutine must route this through
        // `Environment::on_parent_stack_in_coroutine` so it runs on the main
        // stack (matching GL context creation).
        assert!(self.on_main_stack);

        // macOS OpenGL-on-Metal workaround: once the guest app creates its own
        // EAGLContext (a second NSOpenGLContext on our window), a window relayout
        // marks our compositor context's drawable dirty via SDL's `scheduleUpdate`.
        // SDL's `Cocoa_GL_SwapWindow` calls `[nscontext flushBuffer]` *before*
        // applying that pending update, so the flush hits a NULL/stale drawable
        // surface and crashes deep inside SkyLight
        // (`SLSFlushSurfaceWithOptionsAndIndex`). Force the current context to
        // refresh its drawable (`-[NSOpenGLContext update]`, via SDL's override
        // that clears the dirty flag) immediately before the swap.
        #[cfg(target_os = "macos")]
        unsafe {
            let ctx = sdl2_sys::SDL_GL_GetCurrentContext();
            if !ctx.is_null() {
                let sel = sel_registerName(b"update\0".as_ptr() as *const _);
                objc_msgSend(ctx as *mut _, sel);
            }
        }
    ...
    /// Device pixels per point, i.e. 2 when emulating a Retina screen.
    pub fn screen_scale(&self) -> u32 {
        self.screen_scale
    }
    ...
    pub fn size_unrotated_unscaled(&self) -> (u32, u32) {
        // Deliberately 1 for both scales: this is the guest's own coordinate
        // space, in points.
        size_for_orientation(
            self.device_family,
    ...
    pub fn viewport(&self) -> (u32, u32, u32, u32) {
        ...
        // Letterbox within whatever the drawable currently is. When the window
        // still has the app's aspect ratio this picks out the whole drawable, so
        // it is also correct for a freshly-created window; once the user resizes
        // it, the app keeps its aspect ratio and gets bars on the long axis.
        // Note this is the drawable in *pixels*, not the window size in points:
        // with a high-DPI drawable those differ, and rendering into a
        // point-sized viewport would leave the app in a corner.
    ...
    pub fn viewport_y_offset(&self) -> u32 {
        #[cfg(target_os = "macos")]
        {
            let (_, point_h) = self.window.size();
            let (_, pixel_h) = self.window.drawable_size();
            if point_h == 0 {
                return self.viewport_y_offset;
            }
            return self.viewport_y_offset * (pixel_h / point_h).max(1);
        }
