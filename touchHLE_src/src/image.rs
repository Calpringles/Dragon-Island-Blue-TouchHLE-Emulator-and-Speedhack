...
impl Image {
    ...
    /// Encode the image as a PNG file (8-bit RGBA, straight/un-premultiplied
    /// alpha), for `UIImagePNGRepresentation`. Rows are top-to-bottom.
    pub fn to_png(&self) -> Vec<u8> {
        ...
        let pixels = self.pixels(); // premultiplied RGBA, top-to-bottom
        let row_len = width as usize * 4;

        // Raw image data: one filter-type byte (0 = None) per scanline, then the
        // scanline pixels with alpha un-premultiplied (PNG uses straight alpha).
        ...
        for row in pixels.chunks_exact(row_len) {
            ...
            for px in row.chunks_exact(4) {
                let a = px[3];
                match a {
                    0 => raw.extend_from_slice(&[0, 0, 0, 0]),
                    255 => raw.extend_from_slice(px),
                    _ => {
                        let un = |c: u8| {
                            (((c as u32 * 255) + (a as u32 / 2)) / a as u32).min(255) as u8
                        };
                        raw.extend_from_slice(&[un(px[0]), un(px[1]), un(px[2]), a]);
                    }
                }
            }
        }
        ...
        let mut out = Vec::new();
        out.extend_from_slice(&PNG_MAGIC_NUMBER);
