...
impl ClassHostObject {
    ...
    pub(super) fn add_methods_from_bin(
        ...
        // A single method list can legitimately contain the same selector more
        // than once, and then the *first* entry is the one that counts: the real
        // runtime scans the list and stops at the first match. Dragon Island
        // Blue relies on this -- its Dungeon class lists
        // `buildDungeonForLevel:` twice, first the real implementation and later
        // an empty `bx lr` stub -- so letting a later entry win silently turns
        // the method into a no-op.
        //
        // This is only true *within* one list. A category is a separate list
        // added after the class's own, and its methods do override, which still
        // works because that is a separate call to this function.
        let mut added_here = HashSet::new();
        ...
        for i in 0..count {
            ...
            if !added_here.insert(sel) {
                log_dbg!(
                    "Ignoring duplicate {} in one method list; keeping the first implementation",
                    sel.as_str(mem)
                );
                continue;
            }
...
impl ObjC {
    ...
    /// Look up the guest-code IMP (callable address) for a selector on a class,
    /// walking the inheritance chain. Returns `None` if the selector isn't
    /// found or is implemented by a host function (which has no guest address).
    /// Used by `-[NSObject methodForSelector:]`.
    pub fn class_lookup_guest_imp(&self, class: Class, sel: SEL) -> Option<GuestIMP> {
        let mut class = class;
        loop {
            let &ClassHostObject {
                superclass,
