...
/// `GL_POINT_SIZE_ARRAY_OES`. OpenGL ES 1.1 can give every point sprite its own
/// size through this array; desktop OpenGL 2.1 has no equivalent in the fixed
/// function pipeline, so it is emulated in [GLES1OnGL2::DrawArrays] rather than
/// being passed through. It is deliberately not in [ARRAYS], which is the list
/// of arrays the two APIs share.
const POINT_SIZE_ARRAY_OES: GLenum = gles11::POINT_SIZE_ARRAY_OES;
...
pub struct GLES1OnGL2State {
    ...
    /// State of the emulated `GL_POINT_SIZE_ARRAY_OES`.
    point_size_array: PointSizeArray,
}

/// The `GL_POINT_SIZE_ARRAY_OES` client array, which we track ourselves because
/// OpenGL 2.1 has nowhere to put it.
#[derive(Default)]
struct PointSizeArray {
    enabled: bool,
    type_: GLenum,
    stride: GLsizei,
    /// Either a host pointer, or an offset into `buffer` when that is non-zero.
    pointer: usize,
    /// The `GL_ARRAY_BUFFER` binding at the time the pointer was set, or 0 when
    /// the pointer refers to client memory.
    buffer: GLuint,
}
    ...
    fn new(window: &mut Window) -> Result<Self, String> {
        Ok(Self {
            ...
            state: GLES1OnGL2State {
                ...
                point_size_array: Default::default(),
            },
            is_loaded: false,
        })
    }
...
impl GLES1OnGL2<'_> {
    /// Read the point sizes the guest supplied through
    /// `GL_POINT_SIZE_ARRAY_OES` for `count` points starting at `first`.
    unsafe fn read_point_sizes(&mut self, first: GLint, count: GLsizei) -> Vec<GLfloat> {
        let array = &self.state.point_size_array;
        // The array holds either GLfloat or GLfixed (16.16) values.
        let element_size = 4;
        ...
        let is_fixed_point = array.type_ == gles11::FIXED;
        let base = array.pointer;
        let buffer = array.buffer;

        let mut raw = vec![0u8; stride * count as usize];
        if buffer != 0 {
            // The data lives in a buffer object, so it has to be read back.
            let offset = base + stride * first as usize;
            let mut previous_binding: GLint = 0;
            gl21::GetIntegerv(gl21::ARRAY_BUFFER_BINDING, &mut previous_binding);
            gl21::BindBuffer(gl21::ARRAY_BUFFER, buffer);
            gl21::GetBufferSubData(
                gl21::ARRAY_BUFFER,
                offset as gl21::types::GLintptr,
                raw.len() as gl21::types::GLsizeiptr,
                raw.as_mut_ptr() as *mut GLvoid,
            );
            gl21::BindBuffer(gl21::ARRAY_BUFFER, previous_binding as GLuint);
        } else {
            let start = (base + stride * first as usize) as *const u8;
            std::ptr::copy_nonoverlapping(start, raw.as_mut_ptr(), raw.len());
        }

        (0..count as usize)
            .map(|i| {
                let bytes: [u8; 4] = raw[i * stride..i * stride + 4].try_into().unwrap();
                if is_fixed_point {
                    GLfixed::from_ne_bytes(bytes) as GLfloat / 65536.0
                } else {
                    GLfloat::from_ne_bytes(bytes)
                }
            })
            .collect()
    }

    /// Draw points that each have their own size. OpenGL 2.1's fixed function
    /// pipeline can only draw a batch of points at one size, so the batch is
    /// split into runs of equal size — for a particle system, where size varies
    /// smoothly over a particle's life, that is usually far fewer draw calls
    /// than one per point.
    unsafe fn draw_points_with_size_array(&mut self, first: GLint, count: GLsizei) {
        if count <= 0 {
            return;
        }
        let sizes = self.read_point_sizes(first, count);

        let mut previous_size: GLfloat = 0.0;
        gl21::GetFloatv(gl21::POINT_SIZE, &mut previous_size);

        let mut run_start = 0usize;
        for i in 1..=sizes.len() {
            if i < sizes.len() && sizes[i] == sizes[run_start] {
                continue;
            }
            // OpenGL rejects a point size of zero; such points are invisible
            // anyway, so skip the run rather than raising a GL error.
            if sizes[run_start] > 0.0 {
                gl21::PointSize(sizes[run_start]);
                gl21::DrawArrays(
                    gl21::POINTS,
                    first + run_start as GLint,
                    (i - run_start) as GLsizei,
                );
            }
            run_start = i;
        }

        gl21::PointSize(previous_size);
    }
...
impl GLES for GLES1OnGL2<'_> {
    ...
    unsafe fn IsEnabled(&mut self, cap: GLenum) -> GLboolean {
        if cap == POINT_SIZE_ARRAY_OES {
            return self.state.point_size_array.enabled as GLboolean;
        }
        assert!(
            CAPABILITIES.contains(&cap) || ARRAYS.iter().any(|&ArrayInfo { name, .. }| name == cap),
            "Unexpected glIsEnabled({cap:#x})"
        );
        gl21::IsEnabled(cap)
    }
    ...
    unsafe fn EnableClientState(&mut self, array: GLenum) {
        if array == POINT_SIZE_ARRAY_OES {
            // Not a real OpenGL 2.1 array; DrawArrays() emulates it.
            self.state.point_size_array.enabled = true;
            return;
        }
        if CAPABILITIES.contains(&array) {
            ...
            assert!(
                ...
                "Unexpected glEnableClientState({array:#x})"
            );
        }
    ...
    unsafe fn DisableClientState(&mut self, array: GLenum) {
        if array == POINT_SIZE_ARRAY_OES {
            self.state.point_size_array.enabled = false;
            return;
        }
        if CAPABILITIES.contains(&array) {
            log_dbg!(
            ...
            assert!(
                ARRAYS.iter().any(|&ArrayInfo { name, .. }| name == array),
                "Unexpected glDisableClientState({array:#x})"
            );
        }
    ...
    unsafe fn PointSizePointerOES(
        ...
        self.state.point_size_array = PointSizeArray {
            enabled: self.state.point_size_array.enabled,
            type_,
            stride,
    ...
    unsafe fn DrawArrays(&mut self, mode: GLenum, first: GLint, count: GLsizei) {
        ...
        if mode == gl21::POINTS && self.state.point_size_array.enabled {
            self.draw_points_with_size_array(first, count);
        } else {
