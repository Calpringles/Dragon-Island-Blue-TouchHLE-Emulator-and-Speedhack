...
impl Environment {
        ...
        R: 'static,
    {
        ...
        loop {
            // Track which stack we are on, exactly as the main run loop does.
            // Anything the picker draws goes through
            // `on_parent_stack_in_coroutine`, which leaves the flag false on the
            // way out, so without this the poll below trips its own
            // `on_main_stack` assertion on the second iteration.
