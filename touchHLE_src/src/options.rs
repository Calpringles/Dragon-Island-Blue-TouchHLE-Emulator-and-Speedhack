...
pub static SPEED_MULTIPLIER: AtomicU32 = AtomicU32::new(100);
...
pub struct Options {
    ...
    /// Whether to present the guest as a Retina device: `[UIScreen scale]`
    /// returns 2, layers default to a `contentsScale` of 2, and the window is
    /// twice as many pixels per point. Apps that ship only `@2x`/`-hd` art need
    /// this to find their assets at all, so it is not merely cosmetic.
    pub retina: bool,
    pub deadzone: f32,
