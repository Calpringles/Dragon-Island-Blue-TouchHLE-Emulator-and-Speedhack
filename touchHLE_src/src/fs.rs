...
impl Fs {
    ...
    pub fn open<P: AsRef<GuestPath>>(&self, path: P) -> Result<GuestFile, ()> {
        ...
        crate::harness::trace_file_open(path.as_ref().as_str());
    ...
    pub fn open_with_options<P: AsRef<GuestPath>>(
        ...
        // TOUCHHLE_TRACE_FILES=1 logs every file the guest opens. Whether an app
        // picks up its Retina (`-hd`) artwork is otherwise invisible: nothing
        // else in the log names a single loaded file.
        crate::harness::trace_file_open(path.as_str());
