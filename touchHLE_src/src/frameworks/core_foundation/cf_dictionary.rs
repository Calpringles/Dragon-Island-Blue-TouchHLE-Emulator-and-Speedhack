...
fn CFDictionaryCreate(
    ...
    let new: id = msg![env; new initWithKeyCallbacks:key_callbacks andValueCallbacks:value_callbacks];
    for i in 0..num_values {
        let i: crate::mem::GuestUSize = i.try_into().unwrap();
        let key: ConstVoidPtr = env.mem.read(keys + i);
        let value: ConstVoidPtr = env.mem.read(values + i);
        CFDictionarySetValue(env, new, key, value);
    }
    new
}
