...
fn glViewport(env: &mut Environment, x: GLint, y: GLint, width: GLsizei, height: GLsizei) {
    {
        static LAST: std::sync::Mutex<Option<(GLint, GLint, GLsizei, GLsizei)>> =
            std::sync::Mutex::new(None);
        let mut last = LAST.lock().unwrap();
        if *last != Some((x, y, width, height)) {
            *last = Some((x, y, width, height));
            log!("RENDERDIAG glViewport({}, {}, {}, {}) [pre-scale-hack]", x, y, width, height);
        }
    }
    // apply scale hack: assume framebuffer's size is larger than the app thinks
...
// RENDERDIAG: track the current matrix mode so we can log only PROJECTION
// matrix changes (modelview changes every sprite and would flood the log).
static RENDERDIAG_MATRIX_MODE: std::sync::atomic::AtomicU32 =
    std::sync::atomic::AtomicU32::new(0x1700 /* GL_MODELVIEW */);

fn renderdiag_log_projection(env: &Environment, m: ConstPtr<GLfloat>, who: &str) {
    if RENDERDIAG_MATRIX_MODE.load(std::sync::atomic::Ordering::Relaxed) != 0x1701 {
        return; // only care about GL_PROJECTION
    }
    let mut a = [0f32; 16];
    for (i, slot) in a.iter_mut().enumerate() {
        *slot = env.mem.read(m + i as u32);
    }
    static LAST: std::sync::Mutex<Option<[u32; 16]>> = std::sync::Mutex::new(None);
    let key = a.map(|x| x.to_bits());
    let mut last = LAST.lock().unwrap();
    if *last != Some(key) {
        *last = Some(key);
        // For an ortho projection: m[0] = 2/(right-left), m[5] = 2/(top-bottom).
        let w = if a[0] != 0.0 { 2.0 / a[0] } else { 0.0 };
        let h = if a[5] != 0.0 { 2.0 / a[5] } else { 0.0 };
        log!(
            "RENDERDIAG {} PROJECTION: m0={:.5} m5={:.5} tx={:.3} ty={:.3} => ortho size ~{:.1}x{:.1}",
            who, a[0], a[5], a[12], a[13], w, h
        );
    }
}

fn glMatrixMode(env: &mut Environment, mode: GLenum) {
    RENDERDIAG_MATRIX_MODE.store(mode, std::sync::atomic::Ordering::Relaxed);
    with_ctx_and_mem(env, |gles, _mem| {
...
fn glLoadIdentity(env: &mut Environment) {
    if RENDERDIAG_MATRIX_MODE.load(std::sync::atomic::Ordering::Relaxed) == 0x1701 {
        ...
        ONCE.call_once(|| { log!("RENDERDIAG glLoadIdentity on PROJECTION (identity projection at least once)"); });
    }
    with_ctx_and_mem(env, |gles, _mem| {
...
fn glLoadMatrixf(env: &mut Environment, m: ConstPtr<GLfloat>) {
    renderdiag_log_projection(env, m, "glLoadMatrixf");
    with_ctx_and_mem(env, |gles, mem| {
...
fn glMultMatrixf(env: &mut Environment, m: ConstPtr<GLfloat>) {
    renderdiag_log_projection(env, m, "glMultMatrixf");
    with_ctx_and_mem(env, |gles, mem| {
        let m = mem.ptr_at(m, 16);
...
fn glOrthof(
    ...
    {
        static LAST: std::sync::Mutex<Option<[u32; 6]>> = std::sync::Mutex::new(None);
        let key = [
            left.to_bits(), right.to_bits(), bottom.to_bits(),
            top.to_bits(), near.to_bits(), far.to_bits(),
        ];
        let mut last = LAST.lock().unwrap();
        if *last != Some(key) {
            *last = Some(key);
            log!(
                "RENDERDIAG glOrthof(l={}, r={}, b={}, t={}, n={}, f={})  => projection {}x{}",
                left, right, bottom, top, near, far, (right - left).abs(), (top - bottom).abs()
            );
        }
    }
    with_ctx_and_mem(env, |gles, _mem| {
...
fn glFrustumf(
...
) {
    {
        ...
        if *last != Some(key) {
            *last = Some(key);
            log!(
                "RENDERDIAG glFrustumf(l={}, r={}, b={}, t={}, n={}, f={})  => near-plane {}x{}",
                left, right, bottom, top, near, far, (right - left).abs(), (top - bottom).abs()
            );
        }
    }
