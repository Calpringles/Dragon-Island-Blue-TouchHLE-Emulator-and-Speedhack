...
- (bool)renderbufferStorage:(NSUInteger)target
    ...
    let (width, height) = {
        ...
        // The drawable's size in pixels is its bounds (in points) times its
        // contentsScale -- that is how a Retina app gets a Retina drawable, and
        // the app reads the result back with glGetRenderbufferParameterivOES to
        // learn its own resolution. The scale hack is separate: it magnifies the
        // window without the app knowing, so it must not be applied here.
        let contents_scale: CGFloat = msg![env; drawable contentsScale];
        let (rb_w, rb_h) = (
            (width * contents_scale).round() as u32,
            (height * contents_scale).round() as u32,
        );
        log!(
            "renderbufferStorage: CAEAGLLayer bounds {}x{} pts, contentsScale {} => renderbuffer {}x{} px",
            width, height, contents_scale, rb_w, rb_h
        );
        (rb_w, rb_h)
    };
...
fn limit_framerate(next_frame_due: &mut Option<Instant>, options: &Options) -> Option<Duration> {
    let multiplier = crate::options::SPEED_MULTIPLIER.load(std::sync::atomic::Ordering::Relaxed) as f64 / 100.0;
...
unsafe fn present_renderbuffer(env: &mut Environment) {
    ...
    // Release the thread-context borrow of `env` so the buffer swap can be
    // routed onto the main stack: swap_window() asserts on_main_stack, but
    // presentRenderbuffer runs inside the guest coroutine where it is false.
    std::mem::drop(gles_ctx);
    ...
    // Re-acquire the thread context (dropped above) to restore GL state.
    let gles_ctx = super::get_thread_context(
        &mut env.framework_state.opengles,
        &mut env.objc,
        env.current_thread,
