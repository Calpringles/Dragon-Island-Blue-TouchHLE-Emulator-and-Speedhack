...
// Dragon Island Blue (and presumably other apps) never releases its OpenAL
// sources/channels during long play sessions, adding a new one roughly once a
// minute. OpenAL Soft's own default ALC_MONO_SOURCES cap is 256, so after
// ~256 minutes of play the next alGenSources-driven channel creation fails
// and the game crashes (this is the underlying cause of the "opening the
// 257th channel crashes" bug report). Forcing a much higher device source
// limit here doesn't fix the leak itself, but avoids the crash in practice.
const MIN_MONO_SOURCES: ALCint = 4096;
...
fn alcCreateContext(
    ...
    // Collect the guest's requested attributes (if any) into a host-side
    // buffer, so [MIN_MONO_SOURCES] can be enforced below regardless of
    // whether the guest requested nothing, a lower value, or omitted
    // ALC_MONO_SOURCES entirely (in which case the real library would fall
    // back to its own, much lower, default).
    let mut attrs: Vec<(ALCint, ALCint)> = Vec::new();
    if !attr_list.is_null() {
        ...
        while env.mem.read(ptr) != 0 {
            ...
            let value = env.mem.read(ptr + 1);
            log_dbg!("alcCreateContext attribute {:#x} => {}", attr, value);
    ...
    match attrs.iter_mut().find(|(attr, _)| *attr == ALC_MONO_SOURCES) {
        Some((_, value)) if *value < MIN_MONO_SOURCES => {
            *value = MIN_MONO_SOURCES;
        }
        Some(_) => (),
        None => attrs.push((ALC_MONO_SOURCES, MIN_MONO_SOURCES)),
    }
    let mut flat_attrs: Vec<ALCint> = Vec::with_capacity(attrs.len() * 2 + 1);
    for (attr, value) in &attrs {
        flat_attrs.push(*attr);
        flat_attrs.push(*value);
    }
    flat_attrs.push(0); // NULL-terminated
    let attr_list_ptr: *const ALCint = flat_attrs.as_ptr();
