...
pub struct State {
    ...
    /// Global `+[UIView setAnimationsEnabled:]` switch. Stored inverted so the
    /// `Default` (false) corresponds to animations being enabled.
...
pub(super) struct UIViewHostObject {
    ...
    view_controller: id,
    /// For UIWindow: the root view controller (strong reference). nil for
    /// non-window views.
...
+ (())setAnimationsEnabled:(bool)enabled {
    ...
    // This is a global switch independent of any animation transaction block,
    // so it is stored as plain UIKit state rather than routed through the
    // current CATransaction (which may not exist when this is called).
    env.framework_state.uikit.ui_view.animations_disabled = !enabled;
}

+ (bool)areAnimationsEnabled {
    !env.framework_state.uikit.ui_view.animations_disabled
}
...
- (())dealloc {
    ...
    release(env, layer);
    release(env, root_view_controller); // nil for non-window views; no-op then
...
// A view's content scale factor is its layer's contentsScale; cocos2d sets this
// on its EAGLView when enabling Retina, and the drawable is sized from it.
...
- (())setContentScaleFactor:(CGFloat)factor {
    log!("RENDERDIAG [UIView setContentScaleFactor:{}] (ignored, TODO)", factor);
    todo_objc_setter!(this, factor);
}
- (CGFloat)contentScaleFactor {
    log_once!("RENDERDIAG [UIView contentScaleFactor] -> 1.0 (TODO)");
    1.0 // TODO
}
