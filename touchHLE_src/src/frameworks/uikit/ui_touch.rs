...
- (CGPoint)locationInView:(id)that_view { // UIView*
    ...
    let res: CGPoint = if that_view == nil {
        location_in_window
    } else {
        msg![env; that_view convertPoint:location_in_window fromView:window]
    };
    crate::harness::trace_touch("raw", location.x, location.y);
    crate::harness::trace_touch("inWindow", location_in_window.x, location_in_window.y);
    crate::harness::trace_touch("inView", res.x, res.y);
    res
}
