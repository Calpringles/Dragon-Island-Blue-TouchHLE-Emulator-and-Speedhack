...
fn ExtAudioFileGetProperty(
    ...
    // FileLengthFrames isn't an Audio File Services property; compute it from the
    // underlying file. Our AudioFile is always PCM (1 frame per packet), so the
    // frame count equals the packet count.
