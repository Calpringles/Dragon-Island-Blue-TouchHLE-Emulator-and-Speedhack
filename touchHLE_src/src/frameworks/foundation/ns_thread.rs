...
- (bool)isMainThread {
    // True when this thread object is the one currently executing and that
    // thread is the main thread (thread 0). Covers the common
    // `[[NSThread currentThread] isMainThread]` idiom; never reports true for a
    // non-current thread.
    let current: id = msg_class![env; NSThread currentThread];
    this == current && env.current_thread == 0
}
