...
- (bool)scanHexInt:(MutPtr<u32>)result {
    ...
    let tmp: Vec<char> = to_rust_string(env, substring).chars().collect();

    // Skip an optional "0x"/"0X" prefix, then scan hex digits. All characters
    // consumed here are ASCII, so char count == UTF-16 code-unit count == the
    // amount to advance `pos` by.
    let mut idx = 0;
    if tmp.len() >= 2 && tmp[0] == '0' && (tmp[1] == 'x' || tmp[1] == 'X') {
        idx = 2;
    }
    let digits_start = idx;
    let mut value: u32 = 0;
    let mut overflowed = false;
    while idx < tmp.len() && tmp[idx].is_ascii_hexdigit() {
        let digit = tmp[idx].to_digit(16).unwrap();
        match value.checked_mul(16).and_then(|v| v.checked_add(digit)) {
            Some(v) => value = v,
            None => overflowed = true, // keep consuming digits
        }
        idx += 1;
    }
    if idx == digits_start {
        // No hex digits present (possibly a lone "0x"): scan nothing.
        return false;
    }
    // On overflow NSScanner reports UINT_MAX.
    env.mem.write(result, if overflowed { u32::MAX } else { value });

    let new_pos = pos + idx as NSUInteger;
    *env.objc.borrow_mut::<NSScannerHostObject>(this) =
        NSScannerHostObject { to_be_skipped, string, len, pos: new_pos };
    true
}
